...
**A tremendously important step I did not show in the video is to update the endpoints of the servers in the config files of the clients. I had done this when I had set up the demo for the video, and I had built the images with the correct endpoints configured. In case the endpoints are incorrect, the BedREST client will respond with a message like "Failed to connect to backend", or it will simply timeout.**
